"""
Dukascopy Data Downloader with Kaggle Integration - V2
- Descarga datos forex de alta calidad desde Dukascopy
- Sube a Google Drive
- Crea Forrest.py Y Forrest.ipynb con el file_id
- Pushea AMBOS archivos a GitHub con fecha
- Pushea notebook a Kaggle
"""
...
DAYS_TO_DOWNLOAD = int(os.getenv('DAYS_TO_DOWNLOAD', '365'))  # 1 año por defecto
...
def py_to_notebook(py_file, ipynb_file, file_id):
    """Convierte archivo .py a notebook .ipynb"""
    ...
    try:
        ...
        # Dividir en celdas (por comentarios ## o líneas vacías dobles)
        cells = []
        current_cell = []
        
        for line in code.split('\n'):
            # Si encuentra ## crea una nueva celda
        ...
        print(f"✅ Notebook created: {ipynb_file}")
        return ipynb_file
...
def create_forrest_files(file_id, template_path='Forrest.ipynb'):
    """
    Crea Forrest.py y Forrest.ipynb con el file_id de Google Drive
    Añade fecha a los nombres
    """
    ...
    print(f"\n📝 Creating Forrest files with Drive file ID: {file_id}")
    
    try:
        # Leer el template notebook
        with open(template_path, 'r', encoding='utf-8') as f:
            notebook = json.load(f)
        
        # Extraer código del notebook
        code_lines = []
...
'''
        ...
        # 2. Crear archivo .ipynb desde el .py
        py_to_notebook(output_py, output_ipynb, file_id)
        
        # Copiar también como Forrest.py y Forrest.ipynb (sin fecha) para compatibilidad
...
def main():
    print("\n" + "="*80)
    print("🚀 Dukascopy Data Downloader with Kaggle Integration V2")
    print("="*80)
    ...
    try:
        ...
        # Create Forrest files (both .py and .ipynb)
        forrest_py, forrest_ipynb = create_forrest_files(file_id, 'Forrest.ipynb')
        ...
        print("📝 Forrest files created (both .py and .ipynb)")
